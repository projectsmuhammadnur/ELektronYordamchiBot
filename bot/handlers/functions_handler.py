...
async def sell_function(msg: types.Message, state: FSMContext):
    ...
    if msg.text == sell:
        await msg.answer(text="""   Bu yerda siz:
    Xo'jalik mollari
...
async def buy_function(msg: types.Message, state: FSMContext):
    ...
    if msg.text == buy:
        await msg.answer(text="""   Bu yerdan siz yangi xo'jalik va qurilish mollarini, turli xildagi aksessuarlarni xarid qilishingiz mumkin. 
    ...
    Bot orqali xarid qilish jarayonida har ikki tomondan halollik va savdo qonun-qoidalariga rioya qilish talab etiladi. Mahsulot sotib olayotganda sotuvchidan chek talab qilishni unutmang!""",
        ...
        await msg.answer(text="""   Здесь вы можете купить новые товары для дома и строительства, различные аксессуары. 
...
async def offer_function(msg: types.Message, state: FSMContext):
    ...
    if msg.text == offer:
        await msg.answer(text="""   Bu yerda siz o'z takliflaringizni qoldirishingiz mumkin. 
    ...
    Taklifingizni matn formatida yuboring 📄""",
        ...
        await msg.answer(text="""   Здесь вы можете оставить свои предложения. 
...
async def complaint_function(msg: types.Message, state: FSMContext):
    ...
    if msg.text == complaint:
        await msg.answer(text="""   Bu yerda siz o'z shikoyatlaringizni qoldirishingiz mumkin. 
    ...
    Shikoyatingizni matn formatida yuboring 📄""",
        ...
        await msg.answer(text="""   Здесь вы можете оставить свои жалобы. 
...
async def business_card_function(msg: types.Message, state: FSMContext):
    ...
    if msg.text == business_card:
        await msg.answer(text="""   Botimizda o'z xizmat va mahsulotlarini taklif qiluvchi har bir a'zosi o'z vizitkasiga ega bo'lishi shart.
    ...
    Joylashuvingizni tugma orqali yuboring 👇""",
        ...
        await msg.answer(text="""   Каждый участник, предлагающий свои услуги и продукты с помощью нашего бота, должен иметь собственную визитную карточку.

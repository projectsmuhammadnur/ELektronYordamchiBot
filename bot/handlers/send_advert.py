...
from bot.buttons.reply_buttons import back_admin_menu_button, advert_menu_buttons, admin_menu_buttons
...
async def advert_handler(msg: types.Message, state: FSMContext):
    if msg.from_user.id in admins:
        ...
        await msg.answer("Habarni yuboring ❗", reply_markup=await back_admin_menu_button())
...
async def send_forward(msg: types.Message, state: FSMContext):
    if msg.from_user.id in admins:
        ...
        await msg.answer(text=f"📨 Forward xabarni yuboring", reply_markup=await back_admin_menu_button())
